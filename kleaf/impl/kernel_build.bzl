...
def _create_infos(
    ...
    output_group_kwargs["rustavailable"] = depset([ctx.attr.config[KernelConfigInfo].rustavailable_out_dir])
...
def _repack_modules_staging_archive(
    ...
    cmd = hermetic_tools.setup + """
        ...
        find {modules_staging_dir} -type d -empty -print0 | xargs -0r rmdir -p --ignore-fail-on-non-empty || true
